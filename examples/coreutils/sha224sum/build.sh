...
if [ $USE_MUSLLVM == "true" ];
then
    declare -a bitcode=("sha224sum.bc" "libc.a.bc" "libc.a")
else
    declare -a bitcode=("sha224sum.bc" )
fi    
...
MANIFEST=sha224sum.manifest

if [ $USE_MUSLLVM == "true" ];
then
    cat > ${MANIFEST} <<EOF
{"binary": "sha224sum_fin",
"native_libs":[],
"name": "sha224sum",
"static_args": [],
"modules": ["libc.a.bc"],
...
then
    ...
    cp slash/sha224sum_fin sha224sum_slashed
else
