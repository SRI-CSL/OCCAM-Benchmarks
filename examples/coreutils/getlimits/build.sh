...
if [ $USE_MUSLLVM == "true" ];
then
    declare -a bitcode=("getlimits.bc" "libc.a.bc" "libc.a")
else
    declare -a bitcode=("getlimits.bc" )
fi    
...
MANIFEST=getlimits.manifest

if [ $USE_MUSLLVM == "true" ];
then
    cat > ${MANIFEST} <<EOF
{"binary": "getlimits_fin",
"native_libs":[],
"name": "getlimits",
"static_args": [],
"modules": ["libc.a.bc"],
...
then
    ...
    cp slash/getlimits_fin getlimits_slashed
else
