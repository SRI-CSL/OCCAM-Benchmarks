...
if [ $USE_MUSLLVM == "true" ];
then
    declare -a bitcode=("sha384sum.bc" "libc.a.bc" "libc.a")
else
    declare -a bitcode=("sha384sum.bc" )
fi    
...
MANIFEST=sha384sum.manifest

if [ $USE_MUSLLVM == "true" ];
then
    cat > ${MANIFEST} <<EOF
{"binary": "sha384sum_fin",
"native_libs":[],
"name": "sha384sum",
"static_args": [],
"modules": ["libc.a.bc"],
...
then
    ...
    cp slash/sha384sum_fin sha384sum_slashed
else
