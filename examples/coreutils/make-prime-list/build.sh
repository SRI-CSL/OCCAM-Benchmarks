...
if [ $USE_MUSLLVM == "true" ];
then
    declare -a bitcode=("make-prime-list.bc" "libc.a.bc" "libc.a")
else
    declare -a bitcode=("make-prime-list.bc" )
fi    
...
MANIFEST=make-prime-list.manifest

if [ $USE_MUSLLVM == "true" ];
then
    cat > ${MANIFEST} <<EOF
{"binary": "make-prime-list_fin",
"native_libs":[],
"name": "make-prime-list",
"static_args": [],
"modules": ["libc.a.bc"],
"ldflags": [],
"main": "make-prime-list.bc"
}
...
then
    ...
    cp slash/make-prime-list_fin make-prime-list_slashed
else
