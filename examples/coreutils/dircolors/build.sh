...
if [ $USE_MUSLLVM == "true" ];
then
    declare -a bitcode=("dircolors.bc" "libc.a.bc" "libc.a")
else
    declare -a bitcode=("dircolors.bc" )
fi    
...
MANIFEST=dircolors.manifest

if [ $USE_MUSLLVM == "true" ];
then
    cat > ${MANIFEST} <<EOF
{"binary": "dircolors_fin",
"native_libs":[],
"name": "dircolors",
"static_args": [],
"modules": ["libc.a.bc"],
...
then
    ...
    cp slash/dircolors_fin dircolors_slashed
else
