...
if [ $USE_MUSLLVM == "true" ];
then
    declare -a bitcode=("sha256sum.bc" "libc.a.bc" "libc.a")
else
    declare -a bitcode=("sha256sum.bc" )
fi    
...
MANIFEST=sha256sum.manifest

if [ $USE_MUSLLVM == "true" ];
then
    cat > ${MANIFEST} <<EOF
{"binary": "sha256sum_fin",
"native_libs":[],
"name": "sha256sum",
"static_args": [],
"modules": ["libc.a.bc"],
...
then
    ...
    cp slash/sha256sum_fin sha256sum_slashed
else
