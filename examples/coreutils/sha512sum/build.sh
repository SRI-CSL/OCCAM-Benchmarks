...
if [ $USE_MUSLLVM == "true" ];
then
    declare -a bitcode=("sha512sum.bc" "libc.a.bc" "libc.a")
else
    declare -a bitcode=("sha512sum.bc" )
fi    
...
MANIFEST=sha512sum.manifest

if [ $USE_MUSLLVM == "true" ];
then
    cat > ${MANIFEST} <<EOF
{"binary": "sha512sum_fin",
"native_libs":[],
"name": "sha512sum",
"static_args": [],
"modules": ["libc.a.bc"],
...
then
    ...
    cp slash/sha512sum_fin sha512sum_slashed
else
